...
## 天花板高度：场地中颜色较深的有效游戏区域行数（1~20行）
const CEILING: int = 20

## 缓冲区高度：场上方用于临时存放方块的区域（3行），超出此区域则游戏结束
const BUFFER_ZONE_HEIGHT: int = 3

## 场地水平容量（列数）
const H_CAPACITY: int = 10

## 场地垂直容量（行数），等于天花板高度 + 缓冲区高度
...
func _init() -> void:
	if debugMode:
		Debug.printDebug("PlayField: _init() 开始初始化场地")

	_playfield.clear()
	_bounds = Rect2i(0, 0, H_CAPACITY, V_CAPACITY)

	for row in V_CAPACITY:
		var rowArray: Array[Block] = []
		rowArray.resize(H_CAPACITY)
		rowArray.fill(null)
		_playfield.append(rowArray)

	if debugMode:
		Debug.printDebug("PlayField: _init() 场地初始化完成，大小=%sx%s" % [H_CAPACITY, V_CAPACITY])
...
## 绘制网格线
func _drawGrid() -> void:
	var bottomToTop: Vector2 = CEILING * Vector2.UP * CELL_WIDTH

	# 绘制游戏区域网格（0.6透明度）
	var gamePoints: PackedVector2Array = _generateGridPoints(H_CAPACITY + 1, CEILING + 1, Vector2.ZERO)
	draw_multiline(gamePoints, Color(1, 1, 1, 0.6), CELL_LINE_WIDTH)

	# 绘制缓冲区网格（0.2透明度）
	var bufferPoints: PackedVector2Array = _generateGridPoints(H_CAPACITY + 1, BUFFER_ZONE_HEIGHT, bottomToTop)
	draw_multiline(bufferPoints, Color(1, 1, 1, 0.2), CELL_LINE_WIDTH)
...
func _generateGridPoints(cols: int, rows: int, offset: Vector2) -> PackedVector2Array:
	...
	var leftToRight: Vector2 = H_CAPACITY * Vector2.RIGHT * CELL_WIDTH
	...
	for i in rows:
		...
		var right: Vector2 = left + leftToRight
		points.append(left)
		points.append(right)
...
func addBlocks(tetromino: Tetromino, coordinates: Vector2i) -> void:
	if debugMode:
		Debug.printDebug("PlayField: 添加方块到场地，坐标=%s，方块类型=%s" % [coordinates, tetromino.get_class()])
	...
	if debugMode:
		Debug.printDebug("PlayField: 方块已添加到场地")
		...
		for col: int in tetrominoBlocks[row].size():
			if tetrominoBlocks[row][col]:
				...
				if not _bounds.has_point(Vector2i(fieldX, fieldY)):
					if debugMode:
						Debug.printDebug("PlayField: 坐标越界 %s,%s" % [fieldX, fieldY])
					return true

				# 检查是否与已有方块重叠
				if _playfield[fieldY][fieldX] != null:
					if debugMode:
						Debug.printDebug("PlayField: 与已有方块重叠 %s,%s" % [fieldX, fieldY])
					return true
...
func getLockPosition(tetromino: Tetromino, startCoordinates: Vector2i) -> Vector2i:
	...
	if debugMode:
		Debug.printDebug("PlayField: 计算锁定位置，起始坐标=%s" % startCoordinates)
	...
	while dropDistance <= V_CAPACITY:
		...
		if isOverlap(tetromino, testPos):
			...
			var lockPos: Vector2i = startCoordinates + (dropDistance - 1) * Coordinates.down
			if debugMode:
				Debug.printDebug("PlayField: 锁定位置=%s，下落距离=%s" % [lockPos, dropDistance])
			return lockPos
	...
	for row in tetrominoBlocks.size():
		...
		if tetrominoBlocks[row].any(func(ele: int) -> bool: return ele != 0):
			...
			if _playfield[fieldRow].all(func(b: Block) -> bool: return b != null):
				clearLine(fieldRow)
				clearedRows.append(fieldRow)
				clearedLineCount += 1

	if clearedLineCount != 0:
		if debugMode:
			Debug.printDebug("PlayField: 消除%s行，行号=%s" % [clearedLineCount, clearedRows])
...
func clearLine(row: int) -> void:
	if debugMode:
		Debug.printDebug("PlayField: 消除第%s行" % row)
