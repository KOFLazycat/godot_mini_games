...
static var _active: Dictionary = {}  # { node: { TweenFX.TweenFXAnimations.X: tween } }

static func track(node: CanvasItem, anim: TweenFX.TweenFXAnimations, tween: Tween) -> void:
...
static func stop(node: CanvasItem, anim: TweenFX.TweenFXAnimations) -> void:
...
static func is_playing(node: CanvasItem, anim: TweenFX.TweenFXAnimations) -> bool:
	return _active.has(node) and _active[node].has(anim)

static func _on_tween_finished(node: CanvasItem, anim: TweenFX.TweenFXAnimations) -> void:
	if not _active.has(node):
		return
	_active[node].erase(anim)
	if _active[node].is_empty():
